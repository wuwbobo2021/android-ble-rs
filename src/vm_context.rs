...
pub fn android_load_dex(dex_data: &[u8]) -> Global<ClassLoader> {
    ...
    vm.with_env(|env| {
        ...
        if android_api_level() >= 26 {
            ...
            let dex_loader =
                InMemoryDexClassLoader::new_ByteBuffer_ClassLoader(env, dex_buffer, context_loader)
                    .unwrap();
            dex_loader.cast::<ClassLoader>().unwrap().as_global()
        } else {
            let cache_dir = context.getCacheDir().unwrap().unwrap();
            let path_string = cache_dir
                .getAbsolutePath()
                .unwrap()
                .unwrap()
                .to_string_lossy();
            ...
            let dex_file_jstring =
                JString::from_env_str(env, dex_file_path.to_string_lossy().as_ref());
            let oats_dir_jstring =
                JString::from_env_str(env, oats_dir_path.to_string_lossy().as_ref());
...
pub fn jni_load_class_with<'env>(
    loader: Ref<'env, ClassLoader>,
    bin_name: &str,
) -> Option<Global<Class>> {
    let env = loader.env();
